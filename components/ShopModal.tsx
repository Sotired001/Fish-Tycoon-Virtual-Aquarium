...
import { FISH_SPECIES, UPGRADES } from '../constants';
...
const ShopModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'FISH' | 'UPGRADES'>('FISH');
      ...
      <div className="bg-white/90 dark:bg-slate-800/90 w-full max-w-3xl h-[80vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-white/20">
        ...
        <div className="flex border-b border-slate-700">
          <button 
            ...
            className={`flex-1 p-4 font-bold transition-colors ${activeTab === 'FISH' ? 'bg-slate-800 text-amber-400 border-b-2 border-amber-400' : 'text-slate-500 hover:bg-slate-800/50'}`}
          >
            Fish Market ({currentFishCount}/{maxFish})
          </button>
          <button 
            onClick={() => setActiveTab('UPGRADES')}
            className={`flex-1 p-4 font-bold transition-colors ${activeTab === 'UPGRADES' ? 'bg-slate-800 text-amber-400 border-b-2 border-amber-400' : 'text-slate-500 hover:bg-slate-800/50'}`}
          >
            Equipment
          </button>
        </div>
          ...
          {activeTab === 'FISH' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {FISH_SPECIES.map(fish => {
                const ownedCount = ownedFish.find(f => f.speciesId === fish.id)?.count || 0;
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between">
                        <h3 className={`font-bold ${
                          fish.rarity === FishRarity.LEGENDARY ? 'text-amber-500' : 
                  ...
                  <div key={upgrade.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-between shadow-sm">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <button 
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold min-w-[100px] ${
                          isMaxed 
